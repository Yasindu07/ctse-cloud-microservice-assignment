        ...
        <div className="flex items-center justify-between max-w-7xl mx-auto py-4  transition-all">
          ...
          <div className="hidden sm:flex items-center gap-4 lg:gap-8 text-slate-600 font-medium">
            ...
            <form
              ...
              className="hidden xl:flex items-center w-80 text-sm gap-2 bg-slate-100 px-4 py-3 rounded-full focus-within:ring-2 ring-green-500/20 transition"
            >
            ...
            {isAuthenticated && (
              <Link
                href="/cart"
                className="relative flex items-center gap-2 text-slate-600 hover:text-green-600 transition"
              >
                <ShoppingCart size={22} />
                <span className="hidden lg:block">Cart</span>
                <span className="absolute -top-1.5 left-3.5 flex items-center justify-center text-[10px] font-bold text-white bg-red-500 size-5 rounded-full shadow-sm">
                  {cartCount}
                </span>
              </Link>
            )}
              ...
              isAuthenticated ? (
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 bg-red-500 hover:bg-red-600 transition shadow-lg shadow-red-500/30 text-white rounded-full font-semibold text-sm"
                  >
                    Logout
                  </button>
                </div>
            ...
            {mounted ? (
              isAuthenticated ? (
                <div className="flex items-center gap-2">
                  <p className="text-sm text-slate-700 max-w-20 truncate">
                    {user?.name || "User"}
                  </p>
                  <button
                    onClick={handleLogout}
